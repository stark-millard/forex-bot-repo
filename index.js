...
const chekForWithdraw = (dataFromDBwallet, query) => {
	bot.addListener('message', async (msg) => {
		...
		bot.sendMessage(chatId, '✅Ожидайте перевод', {
...
⚠️ 🛠 Тех.Поддержка - @ForexSupportUA`,
		...
		if (query.data === 'withdraw') {
			...
			await bot.sendMessage(
				chatId,
				`
🤵 Для вывода средств УКАЖИТЕ СУММУ В СЛЕДУЮЩЕМ СООБЩЕНИИ:\n
💬 Комментарий : FRX-INV:${query.message.chat.id}\n
...
Верификация проходится путём отсылания фото вашего паспорта, для подтверждения личности и возраста.`,
				{
					reply_markup: deleteKeyboard
				}
			)
